...
export const Products = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    let res = await fetch(
      `https://faballey-arc8.onrender.com/products?_page=${currentPage}&_limit=12&${
        sort != "" ? "_sort=price1&_order=" + sort : ""
      }&${filters.category != "" ? "category=" + filters.category : ""}&${
        filters.color != "" ? "color=" + filters.color : ""
      }&${filters.sleeves != "" ? "sleeves=" + filters.sleeves : ""}&${
        filters.length != "" ? "length=" + filters.length : ""
      }&${filters.discount != "" ? "discount_lte=" + filters.discount : ""}&`
    );
  ...
  const postData = async (propdata) => {
    ...
    delete newData.id;
    newData.qty = 1;
    let res = await fetch(`https://black-cormorant-hat.cyclic.app/cart`, {
      method: "POST",
  ...
  const addToCart = async (e, propdata) => {
    ...
    let res = await fetch(
      `https://faballey-arc8.onrender.com/cart?uniqueId=${propdata.uniqueId}`
    );
    let data = await res.json();
    console.log(data);
    if (data.length > 0) {
